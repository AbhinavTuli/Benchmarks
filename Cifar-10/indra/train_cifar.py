...
def train_transform(sample):
    ...
    img = img.type(ch.float16)
    img = Normalize(mean=CIFAR_MEAN, std=CIFAR_STD)(img)
    label = sample['labels']
    return img, label
...
def make_dataloaders(train_dataset=None, val_dataset=None, batch_size=None, num_workers=None):
    ...
    datasets = {
        'train': api.dataset(paths['train']),
        'test': api.dataset(paths['test'])
    }
    ...
    for name in ['train', 'test']:
        transform = train_transform if name == 'train' else test_transform
        loaders[name] = Loader(datasets[name], batch_size=batch_size, num_workers=num_workers,
